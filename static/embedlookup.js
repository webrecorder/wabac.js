...
async function initTemplates() {
  const templates = document.querySelectorAll(
    "template[data-archive-name][data-archive-file]",
  );
  ...
  if (swAvail) {
    navigator.serviceWorker.addEventListener("message", (event) => {
      if (event.data.msg_type === "collAdded") {
        ...
        const replayOrigin =
          iframe.parentElement.parentElement.getAttribute(
            "data-replay-origin",
  ...
  for (let template of templates) {
    const fileURL = new URL(
      template.getAttribute("data-archive-file"),
      window.location.origin,
    ).href;
  ...
  <div class="emp-container emp-live" style="display: none">
    ...
    const liveNode = liveIframe.contentDocument.importNode(
      template.content,
      true,
    );
    if (liveIframe.contentDocument.readyState === "complete") {
      ...
      liveIframe.contentWindow.addEventListener(
        "DOMContentLoaded",
        () => {
          liveIframe.contentDocument.body.appendChild(liveNode);
        },
  ...
  window.addEventListener("message", (event) => {
    ...
    for (var iframe of iframes) {
      if (iframe.src.indexOf(event.data.url) > 0) {
        iframe.setAttribute(
          "data-ts",
          "Archived On: " + tsToDate(event.data.ts),
        );
        iframe.parentElement.parentElement
          .querySelector("a.emp-tab.emp-archived")
          .click();
      }
    }
  });
...
async function initSW(relUrl, path) {
  ...
  if (!navigator.serviceWorker) {
    ...
    if (loc.protocol === "http:") {
      msg =
        "Service workers only supported when loading via https://, but this site loaded from: " +
        loc.origin;
    } else {
