...
export class LiveProxy {
    ...
    async getResource(request, prefix) {
        ...
        if (isPOST && response.status >= 400) {
            void this.sendProxyError("post-request-failed", url, request.method, response.status);
        }
        else if (response.status === 429) {
            void this.sendProxyError("rate-limited", url, request.method, response.status);
        }
    ...
    async sendProxyError(type, url, method, status) {
