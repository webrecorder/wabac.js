...
import { DomainSpecificRuleSet, hasRangeAsQuery, HTML_ONLY_RULES, } from "./dsruleset";
...
export class Rewriter {
    ...
    workerInsertFunc;
    _jsonpCallback;
    constructor({ baseUrl, prefix, responseUrl = undefined, workerInsertFunc = null, headInsertFunc = null, urlRewrite = true, contentRewrite = true, decode = true, useBaseRules = false, }) {
    ...
    getRewriteMode(request, response, url = "", mime = "") {
        // [TODO]
...
export class ProxyRewriter extends Rewriter {
    ...
    rewriteUrl(urlStr) {
        ...
        if (this.proxyTLD && this.localTLD && urlStr.indexOf(this.proxyTLD) > 0) {
            ...
            if (url.host.endsWith(this.proxyTLD)) {
                const host = url.host.slice(0, -this.proxyTLD.length).replace(".", "-") +
                    this.localTLD;
                const newUrl = this.localScheme + "//" + host + url.href.slice(url.origin.length);
                return newUrl;
            }
        }
        if (this.httpToHttps) {
    ...
    fullRewriteUrl(urlStr, forceAbs = false) {
