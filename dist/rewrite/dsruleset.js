...
export const DEFAULT_RULES = [
    {
        ...
        rxRules: [
            [
                /ytplayer.load\(\);/,
                ruleReplace('ytplayer.config.args.dash = "0"; ytplayer.config.args.dashmpd = ""; {0}'),
            ],
            [
...
export function hasRangeAsQuery(url) {
    if (!url) {
        return null;
    }
...
export function removeRangeAsQuery(url) {
    ...
    try {
        ...
        if (!parsedUrl.searchParams.has(result.start) ||
...
export function ruleRewriteFBDash(text, opts) {
    const START_TAG = "\\u003C?xml";
    const END_TAG = "/MPD>";
    ...
    const rwtext = JSON.parse('"' + text.slice(start, end) + '"');
...
// ===========================================================================
function ruleReplace(str) {
    return (x) => str.replace("{0}", x);
}
// ===========================================================================
function ruleReplacePad(replacement) {
    return (matched) => {
...
// ===========================================================================
function ruleRewriteVimeoConfig(str) {
    let config;
    try {
...
// [TODO]
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function ruleRewriteVimeoDashManifest(str, opts) {
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (!opts) {
        return str;
    ...
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    vimeoManifest.video, maxBitrate, "video/mp4");
    vimeoManifest.audio = filterByBitrate(
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    vimeoManifest.audio, maxBitrate, "audio/mp4");
...
//# sourceMappingURL=dsruleset.js.map