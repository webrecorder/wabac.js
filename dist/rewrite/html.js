...
export class HTMLRewriter {
    ...
    rewriteMetaContent(attrs, attr, rewriter) {
        ...
        if (equiv === "content-security-policy") {
            ...
            return attr.value.replace(META_REFRESH_REGEX, (m, p1, p2, p3) => p1 + this.rewriteUrl(rewriter, p2, false, "mt_") + p3);
        }
    ...
    rewriteTagAndAttrs(tag, attrRules, rewriter) {
        ...
        for (const attr of tag.attrs) {
            ...
            // but disabling triggers default behavior which often does
                ...
                attr.value = this.rewriteUrl(rewriter, attr.value, false, attrRules[name]);
            }
        }
    }
    getAttr(attrs, name) {
    ...
    rewriteUrl(rewriter, text, forceAbs = false, mod = "") {
...
export class ProxyHTMLRewriter extends HTMLRewriter {
    rewriteUrl(rewriter, text, forceAbs = false, mod = "") {
        if (mod === "if_" || mod === "fr_" || mod === "mt_") {
            return rewriter.fullRewriteUrl(text, forceAbs);
        }
        if (mod === "ln_") {
            // if html charset not utf-8, just convert the url to utf-8 for rewriting
            if (!this.isCharsetUTF8) {
