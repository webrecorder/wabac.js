...
class WARCLoader extends BaseParser {
    ...
    parseRecords(record, reqRecord) {
        ...
        if (this.pageMap[ts + "/" + url] && payload && mime.startsWith("text/")) {
            ...
            // @ts-expect-error [TODO] - TS2345 - Argument of type 'string | undefined' is not assignable to parameter of type 'string'.
            url, payload, headers.get("content-encoding"), headers.get("transfer-encoding"));
        }
    ...
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async load(db, progressUpdate, totalSize = 0) {
        ...
        try {
            for await (const record of parser) {
                ...
                // [TODO]
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const interruptLoads = self.interruptLoads;
                // [TODO]
            ...
            `Sorry there was an error downloading. Please try again (${e})`, parser.offset, totalSize);
            console.warn(e);
        }
...
//# sourceMappingURL=warcloader.js.map