...
import { addProxyAllowPaths, getCSP, isAjaxRequest, proxyAllowPaths, updateCSP, } from "./utils";
...
import { ArchiveRequest, resolveFullUrlFromReferrer, } from "./request";
...
export class SWCollections extends WorkerLoader {
    ...
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    metadata, extraConfig = {}, type = "archive") {
        // [TODO]
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
...
export class SWReplay {
    ...
    isFromReplay(request) {
        return (request.url.startsWith(this.replayPrefix) ||
    ...
    async wrapCSPForFrame(resp, request) {
    ...
    async defaultFetch(request, opts = {}) {
        if (!opts.cache &&
    ...
    async getResponseFor(request, event) {
        ...
        if (response) {
            if (this.stats) {
                ...
                response, response.status, request, event);
            }
...
//# sourceMappingURL=swmain.js.map