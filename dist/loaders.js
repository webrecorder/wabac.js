...
import { Canceled, MAX_FULL_DOWNLOAD_SIZE, randomId, AuthNeededError, DeleteExpiredError, sleep, } from "./utils";
...
// [TODO]
// eslint-disable-next-line @typescript-eslint/no-explicit-any
self.interruptLoads = interruptLoads;
// ===========================================================================
export class CollectionLoader {
    ...
    async deleteExpireMultiWACZs(stores) {
    ...
    async _initColl(data, storeMultiWACZ = null) {
...
export class WorkerLoader extends CollectionLoader {
            ...
            case "addColl": {
                ...
                try {
                    ...
                    if (e instanceof AuthNeededError) {
                        console.warn(e);
                        progressUpdate(0, "permission_needed", null, null, 
    ...
    progressUpdate) {
        ...
        const config = { root: data.root || false };
        let generalDB = null;
        ...
        if (file.sourceUrl.startsWith("proxy:")) {
            ...
            let sourceName = file.name || file.sourceUrl;
            ...
            const { abort, response } = await sourceLoader.doInitialFetch(sourceExt === ".wacz", false);
            if (!sourceExt) {
            ...
            if (!sourceLoader.isValid) {
                const text = sourceLoader.length && sourceLoader.length <= 1000
                    ? await response.text()
                    : "";
                progressUpdate(0, `\
...
Make sure this is a valid URL and you have access to this file.`);
            ...
            if (updateExistingConfig) {
                ...
                // [TODO]
                // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
                file.importCollId, contentLength, contentLength, updateExistingConfig.decode);
