...
import { tsToDate, isNullBodyStatus, makeHeaders, digestMessage, getTS, getStatusText, randomId, PAGE_STATE_SYNCED, } from "./utils";
...
export class ArchiveDB {
    ...
    async addVerifyData(_prefix = "", _id, _expected, _actual = null, _log = false) {
        return;
    }
    async addVerifyDataList(_prefix, _datalist) {
        return;
    }
    ...
    async getResource(request, _prefix, event, opts = {}) {
    ...
    async loadPayload(result, _opts) {
        if (result.digest && !result.payload) {
            if (result.digest === EMPTY_PAYLOAD_SHA256 ||
    ...
    async resourcesByUrlAndMime(url, mimes, count = 1000, prefix = true, fromUrl = "", fromTs = "") {
        ...
        const fullResults = await this.db.getAll("resources", this.getLookupRange(url, prefix ? "prefix" : "exact", fromUrl, fromTs), queryCount);
    ...
    async resourcesByMime(mimesStr, count = 100, fromMime = "", fromUrl = "", fromStatus = 0) {
...
//# sourceMappingURL=archivedb.js.map