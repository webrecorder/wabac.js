...
export class ZipRangeReader {
    ...
    async load(always = false) {
        if (!this.entries || always) {
            ...
            const endChunk = (await this.loader.getRangeFromEnd(endChunkOffset, false));
            const start = Math.max((await this.loader.getLength()) - endChunkOffset, 0);
            try {
                this.entries = this._loadEntries(endChunk, start);
            }
            catch (e) {
                if (e instanceof LoadMoreException) {
                    const extraChunk = (await this.loader.getRange(e.start, e.length, false));
    ...
    async loadFile(name, { offset = 0, length = -1, signal = null, unzip = false, computeHash = false, } = {}) {
        ...
        if (entry.offset === undefined) {
            const header = (await this.loader.getRange(entry.localEntryOffset, 30, false));
...
//# sourceMappingURL=ziprangereader.js.map