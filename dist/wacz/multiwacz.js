...
import { AccessDeniedError, digestMessage, handleAuthNeeded, tsToDate, getTS, sleep, DeleteExpiredError, RangeError, } from "../utils";
import { getSurt } from "warcio";
import { LiveProxy } from "../liveproxy";
import { INDEX_CDX, INDEX_IDX, INDEX_NOT_LOADED, NO_LOAD_WACZ, WACZFile, WACZ_LEAF, } from "./waczfile";
...
export class MultiWACZ extends OnDemandPayloadArchiveDB {
    ...
    async addVerifyData(prefix = "", id, expected, actual = null, log = false) {
        let matched = false;
        if (prefix) {
            id = prefix + id;
        }
    ...
    async loadCDX(filename, waczname, 
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    async loadIDX(filename, waczname, 
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    progressUpdate, total) {
        const { reader, hasher } = await this.loadFileFromNamedWACZ(waczname, filename, { computeHash: true });
    ...
    async lookupUrlForWACZ(waczname, url, datetime, 
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    opts) {
    ...
    async loadFileFromWACZ(waczfile, filename, 
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    opts, numRetries = 0) {
        try {
            ...
            if (numRetries <= MAX_FILE_LOAD_RETRIES && (await this.retryLoad(e))) {
                return await this.loadFileFromWACZ(waczfile, filename, opts, numRetries + 1);
            }
            else {
                throw e;
            }
    ...
    async loadFileFromNamedWACZ(waczname, filename, 
    // [TODO]
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    opts) {
    ...
    async computeFileHash(waczname, hash) {
        if (!hash) {
    ...
    async addNewWACZ({ name, hash, path, crawlId, parent, loader = null,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
     }) {
    ...
    async addInitialPages(pagesImport) {
        const pages = [];
        for (const { id, url, title, ts, mime, status, depth, favIconUrl, filename, isSeed, crawl_id, } of pagesImport) {
            ...
            if (isSeed) {
                const set = this.seedPageWACZs.get(crawl_id || "") || new Set();
                set.add(filename);
    ...
    { pageId, noRedirect } = {}) {
        ...
        const waczFilesToTry = await this.getWACZFilesToTry(request, waczname);
        ...
        if (foundMap.size > 0) {
            ...
            return Response.redirect(`${prefix}:${foundHash}/${timestamp}mp_/${request.url}`);
        }
    ...
    async queryPages(search = "", page = 1, pageSize = 25) {
    ...
    async getWACZFilesToTry(request, waczname) {
        let names = [];
    ...
    async getWACZFilesForPagesQuery(requestUrl) {
        const selectFiles = [];
        if (!this.pagesQueryUrl) {
            return null;
        }
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        this.addInitialPages(json.items).catch((e) => console.log(e, "additional page add failed"));
        return selectFiles;
    }
    async checkUpdates() {
        if (this.rootSourceType !== "json") {
    ...
    async loadFromJSON(response = null) {
        ...
        if (!response || (response.status !== 206 && response.status !== 200)) {
            console.warn("WACZ update failed from: " +
...
//# sourceMappingURL=multiwacz.js.map