    ...
    async handleApi(request, params, event) {
        ...
        switch (params._route) {
            ...
            case "createColl": {
                ...
                const coll = await this.collections.initNewColl(requestJSON.metadata || {}, 
                // [TODO]
                // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
            ...
            case "curated": {
                ...
                const curated = await coll.store.db.getAllFromIndex("curatedPages", "listPages", IDBKeyRange.bound([list], [list + 1]));
                return { curated };
            }
            case "deletePage": {
                // @ts-expect-error [TODO] - TS4111 - Property 'coll' comes from an index signature, so it must be accessed with ['coll'].
