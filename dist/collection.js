...
import { getTS, getSecondsStr, parseSetCookie, handleAuthNeeded, REPLAY_TOP_FRAME_NAME, proxyAllowPaths, getCSP, } from "./utils";
...
export class Collection {
    ...
    async handleRequest(request, event) {
        ...
        if (!response) {
            ...
            return notFoundByTypeResponse(request.request, requestURL, requestTS, this.liveRedirectOnNotFound);
        }
        ...
        if (!response.noRW) {
            if (!request.isProxyOrigin) {
                response = await this.fullRewrite(request, response, baseUrl, requestURL, requestTS);
            }
            else {
                response = await this.proxyRewrite(request, response, baseUrl, requestTS);
            }
        }
    ...
    async fullRewrite(request, response, baseUrl, requestURL, requestTS) {
        ...
        const headInsertFunc = (url) => {
            ...
            return this.makeHeadInsert(url, requestTS, topUrl, basePrefix, arResponse, request.referrer);
        };
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const workerInsertFunc = (text, opts) => {
            if (opts?.isModule) {
                return text;
            }
            return (`
    ...
    async proxyRewrite(request, response, baseUrl, requestTS) {
        const basePrefix = this.prefix + (request.pageId ? `:${request.pageId}/` : "");
        ...
        const headInsertFunc = (url) => {
            const presetCookieStr = this.getCookiePreset(response, request.proxyScheme);
...
<!-- End WB Insert -->
    ...
    getCookiePreset(response, scheme) {
    ...
    getCanonRedirect(query) {
        ...
        if (schemeRel) {
            const scheme = referrer && referrer.indexOf("/http://") > 0 ? "http:" : "https:";
            url = scheme + url;
        }
        try {
...
`;
    ...
    makeHeadInsert(url, requestTS, topUrl, prefix, response, referrer) {
        ...
        const pixelRatio = extraOpts && Number(extraOpts.pixelRatio) ? extraOpts.pixelRatio : 2;
