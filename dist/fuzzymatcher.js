...
export class FuzzyMatcher {
    ...
    getRuleFor(reqUrl) {
        ...
        for (const testRule of this.rules) {
            ...
            if (matchUrl.length < MAX_ARG_LEN * 4 &&
    ...
    fuzzyCompareUrls(reqUrl, results, matchedRule) {
        ...
        if (matchedRule?.replace !== undefined &&
    ...
    fuzzyBestMatchQuery(reqUrlStr, results, rule) {
        let reqUrl;
        try {
        ...
        const reqArgs = rule?.args && !rule.fuzzyArgs ? new Set(rule.args[0]) : null;
